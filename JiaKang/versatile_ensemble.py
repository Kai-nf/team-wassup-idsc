...
# ── File paths ────────────────────────────────────────────────────────────────
JSON_MODEL_A = Path("felicia/results/evaluation_metrics/method4_rf_smote.json")   # can be changed, now is rf
...
W_MODEL_A = 0.4   # rf  weight
W_MODEL_B = 0.6   # CNN weight
...
SCHEMA_CONFIG = {
    "model_a": {   # LR (v4) — awaiting regeneration
        ...
        "y_prob":         "y_prob",
        "y_true":         "y_true",
        "y_prob_MISSING": False,         # ← set False once JSON is regenerated
    },
    "model_b": {   # CNN (v3.1)
        "label":          "1D-CNN (v3.1)",
        "folds_root":     "folds",
...
def main():
    ...
    out_path = Path("results") / "ensemble_soft_vote.json"
